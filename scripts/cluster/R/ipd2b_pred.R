...
print(jobid)
args <- read.table("scripts/cluster/args/frequentist/ipd2b_frequentist_pred.txt"
                   , header = T, stringsAsFactors = F)[jobid,]
print(args)

ipd2b_mod_fun <- function(trait, outcome, type, mod, cov){
  # file <- sprintf("data/one_stage/%s_%s.RData", trait, outcome); print(file)
  # ## load the data
  # load(file)
  ...
  if(type == "Bayesian") load("results/2b_ipd_mlm/bayes_sample_mod.RData")
  ...
  #                                                                , warmup = 1000, refresh = 0, cores = 4)}
  # save(m, file = sprintf("results/2b_ipd_mlm/%s/models/%s_%s_%s_%s.RData"
  #                        , type, outcome, trait, mod, cov))
  
  file <- sprintf("results/2b_ipd_mlm/%s/models/%s_%s_%s_%s.RData", type, outcome, trait, mod, cov); print(file)
  load(file)
  ...
  # rx <- std_eff_fun(m, type); head(rx)
  # save(fx, rx, file = sprintf("results/2b_ipd_mlm/%s/summary/%s_%s_%s_%s.RData"
  #                             , type, outcome, trait, mod, cov))
  
  ## extract heterogeneity estimates
  # het <- ipd2b_hetero_fun(m, type)
  # save(het, file = sprintf("results/2b_ipd_mlm/%s/heterogeneity/%s_%s_%s_%s.RData"
  #                          , type, outcome, trait, mod, cov))
  
  ## simple effects for moderators
  if(mod != "none"){
    pred.fx <- ipd2b_fx_pred_fun(m, mod, type)
    pred.rx <- if(mod %in% stdyModers) NULL else ipd2b_rx_pred_fun(m, mod, type)
    save(pred.fx, pred.rx, file = sprintf("results/2b_ipd_mlm/%s/predicted/%s_%s_%s_%s.RData"
                                          , type, outcome, trait, mod, cov))
  }
...
ipd2b_hetero_fun <- function(m, type){
  ...
  if(type == "Frequentist") args$conf.method = "boot"; args$nsim <- 100
  do.call(tidy, args) %>%
...
ipd2b_mod_fun(args[,1], args[,2], args[,3], args[,4], args[,5])